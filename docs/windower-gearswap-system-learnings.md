...
- For **weapon mode** vs **weapon_lock**: preserve our current precedence (mode trumps lock on switch; lock strips weapon slots from non‑weapon equips) and surface explicit echo lines for clarity.
...
2025-11-01 20:17 PT — Run 1

**Focus:** Windower core + GearSwap user hooks and variables; practical idioms for AutoLoader/GearSwap.

### GearSwap user event flow (high-signal order)
- `get_sets()`: one-time init; define `sets` and require libs. Good place to `include('Mote-Include.lua')` *if* using Mote, or to set up local tables for a pure (non‑Mote) file.
- `pretarget(spell)`: command text reached buffer; can `cancel_spell()` or `change_target('<stpt>')` before packets.
- `precast(spell, position)`: right before outgoing action packet; safe point for **Fast Cast**/**JA lock**; can `verify_equip()`, `cast_delay(n)`, or force a slight send delay.
- `midcast(spell)`: immediately after the outgoing packet is injected; equip midcast sets (mag acc/WS mods, etc.).
- `aftercast(spell)`: on result/interrupt; return to idle/engaged.
- Other common hooks: `status_change(new, old)`, `buff_change(name, gain, buff_details)`, `buff_refresh(name, buff_details)`, `self_command(cmd)`, `pet_*` variants, `sub_job_change(new, old)`, `party_buff_change(...)`, `indi_change(...)`, `file_unload(file_name)`.

### GearSwap core helpers to remember
- `set_combine(set1, set2, ...)` — right-most wins.
- `equip(set1, set2, ...)` — builds + equips; later calls override earlier.
- `change_target(new_target)` — only valid in **pretarget**.

### High‑value variables exposed by GearSwap
- `spell` (name/prefix/type/skill/element/mp_cost/tp_cost/range/recast/recast_id/cast_time/wsA/B/C/interrupted)
- `spell.target` (name, raw, type, hpp, distance, isallymember, is_npc, tp/mpp if available, pos/facing, index/id/claim_id)
- `player` (name/status/hp,mp,tp,hpp,mpp, main_job/_id/_full, race/_id, status_id)
- `player.equipment.slot` (main/sub/range/ammo/head/neck/…/feet)

### Idioms & patterns (vanilla GS)
- Pure GS skeleton (non‑Mote):
```lua
function get_sets()
  sets = {}
  sets.precast = {head='Carmine Mask +1'}
  sets.midcast = {body='Jhakri Robe +2'}
  sets.aftercast = {idle={legs='Carmine Cuisses +1'}}
end

function pretarget(spell)
  if spell.type == 'WhiteMagic' and spell.target.is_npc then cancel_spell() end
end
...
function midcast(spell)
  if spell.action_type == 'Magic' then equip(sets.midcast) end
end

function aftercast(spell)
  if player.status == 'Engaged' then equip(sets.engaged or {}) else equip(sets.aftercast.idle or {}) end
end

function status_change(new, old)
  if new == 'Engaged' then equip(sets.engaged or {}) else equip(sets.aftercast and sets.aftercast.idle or {}) end
end

function self_command(cmd)
  if cmd == 'toggle-acc' then state.Acc = not state.Acc end
end
```
- With Mote‑Include: prefer `job_precast/job_midcast/job_post_midcast/job_aftercast/status_change/buff_change` to customize; manage `state.OffenseMode/HybridMode/IdleMode` etc.; use `set_combine()` heavily.
- Keybinds via console from your GS file (cleanup in `file_unload`):
```lua
send_command('bind ^f9 gs c cycle OffenseMode')
send_command('bind !f9 gs c cycle HybridMode')
function file_unload() send_command('unbind ^f9; unbind !f9') end
```

### Windower core addon API touchpoints useful to GS/AutoLoader
- Registering to engine events from addons:
```lua
windower.register_event('addon command', function(cmd, ...)
  -- parse args and dispatch
end)
windower.register_event('incoming chunk', function(id, data) -- see packets note below end)
windower.register_event('outgoing chunk', function(id, data) -- can modify/return new data end)
```
- Console helpers: `windower.add_to_chat(color, text)`, `windower.send_command('bind ...')`.
- FFXI handles: `windower.ffxi.get_player()`, `windower.ffxi.get_mob_by_target('t')`.
- Libraries: `local res = require('resources')` (items, spells, jobs, buffs, etc.), `local packets = require('packets')` (parse/new/inject).

### Packets quick ref (for adjunct addons)
- **Incoming/Outgoing** hooks fire for raw packet bytes; use `packets.parse('incoming', data)` to table‑ify, or build with `packets.new()`/inject with `packets.inject()`.
- Blocking/modifying is cooperative across addons; design conservatively, and prefer *modify and return* over hard‑blocking where possible.

### Best practices (field‑tested)
- Keep **pretarget/precast** lightweight; avoid expensive table builds there—precompute sets.
- Always fall back in `aftercast` and mirror in `status_change` for robustness against interruptions.
- Centralize mode toggles (Offense/Hybrid/Idle/Ranged/etc.) and expose via `self_command` + binds.
- In `buff_change`, gate frequent‑fire buffs (e.g., DoTs) to avoid churn; debounce via `coroutine.schedule` if needed.
- In `file_unload`, **unbind** keys you bound during load.
- Prefer `set_combine(base, variant)` over mutating base tables to keep diff‑like clarity.

### AutoLoader cross‑links
- Use Windower `lua load|unload|reload <addon>` and `gs export` tooling to bootstrap per‑job configs.
- AutoLoader can listen for job changes (GS emits `sub_job_change`; for main job changes detect via `player.main_job` polling or a lightweight addon watching relevant packets) and then `lua l <helper>` / `gs reload` target files.
- Keep per‑job data in `addons/gearswap/data/<name>_<job>.lua`; AutoLoader can compute paths and check presence before invoking GS reload.

### Notes / caveats
- `change_target()` only valid in `pretarget`.
- Packet blocking behavior depends on engine cooperation across addons; avoid relying on `return true` semantics from very old examples.
- Some Mote docs/examples are community‑maintained; verify versions of `Mote-*.lua` in `addons/gearswap/libs/` vs your job files.
